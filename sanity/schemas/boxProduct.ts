...
export const boxProductSchema = {
  ...
  type: "document",
  fields: [
    { name: "id",        title: "מזהה",        type: "slug",   validation: (R: Rule) => R.required(), options: { source: "name" } },
    { name: "name",      title: "שם המוצר",    type: "string", validation: (R: Rule) => R.required() },
    { name: "unitPrice", title: "מחיר ליחידה ₪", type: "number", validation: (R: Rule) => R.required().min(0) },
